pathD=['H:\WM_data\HC\BOLDextract\Txtoutput11\r_value_results4\''partialFC.mat'];
sbj = importdata('H:\WM_data\HC\BOLDextract\Txtoutput4\HCsbj.txt');
partialFC=cell(88,1);
for t=1:length(sbj)
    data=importdata(['H:\WM_data\HC\BOLDextract\Txtoutput11\partialFC\' sbj{t} '.mat']);
    partialFC{t,1}=data;
end
...
for i=1:88
    for j=1:24
        if j==1
            ALI_pre_L(i,j)=dot(abs(partialFC{i,1}(2:24,j)),All_ALI(i,2:24))/sum(abs(partialFC{i,1}(2:24,j)));
        else
            ALI_pre_L(i,j)=dot(abs(partialFC{i,1}([1:j-1,j+1:24],j)),All_ALI(i,[1:j-1,j+1:24]))/sum(abs(partialFC{i,1}([1:j-1,j+1:24],j)));
        end
    end
end
...
for i=1:88
    for j=1:24
        if j==1
            ALI_pre_L_2(i,j)=dot(abs(partialFC{i,1}(2:24,j+24)),All_ALI(i,2:24))/sum(abs(partialFC{i,1}(2:24,j+24)));
        else
            ALI_pre_L_2(i,j)=dot(abs(partialFC{i,1}([1:j-1,j+1:24],j+24)),All_ALI(i,[1:j-1,j+1:24]))/sum(abs(partialFC{i,1}([1:j-1,j+1:24],j+24)));
        end
    end
end
...
for i=1:88
    for j=1:24
        if j==1
            ...
            ALI_pre_R(i,j)=dot(abs(partialFC{i,1}(26:48,j+24)),All_ALI(i,2:24))/sum(abs(partialFC{i,1}(26:48,j+24)))  ;
        else
           
            ALI_pre_R(i,j)=dot(abs(partialFC{i,1}([25:j+23,j+25:48],j+24)),All_ALI(i,[1:j-1,j+1:24]))/sum(abs(partialFC{i,1}([25:j+23,j+25:48],j+24)));
        end
    end
end
...
for i=1:88
    for j=1:24
        if j==1
            ...
            ALI_pre_R_2(i,j)=dot(abs(partialFC{i,1}(26:48,j)),All_ALI(i,2:24))/sum(abs(partialFC{i,1}(26:48,j)))  ;
        else
           
            ALI_pre_R_2(i,j)=dot(abs(partialFC{i,1}([25:j+23,j+25:48],j)),All_ALI(i,[1:j-1,j+1:24]))/sum(abs(partialFC{i,1}([25:j+23,j+25:48],j)));
        end
    end
end
...
for i=1:88
    for j=1:48
        if j == 1
          ...
          ALI_pre(i,j)=dot(abs(partialFC{i,1}([2:24 26:48],1)),allALI(1,[2:24 26:48]))/sum(abs(partialFC{i,1}([2:24 26:48],1))); 
        elseif j<=24
           
           ALI_pre(i,j)=dot(abs(partialFC{i,1}([1:(j-1) (j+1):(j+23) (j+25):48],j)),allALI(i,[1:(j-1) (j+1):(j+23) (j+25):48]))/sum(abs(partialFC{i,1}([1:(j-1) (j+1):(j+23) (j+25):48],j)));
        else 
           ALI_pre(i,j)=dot(abs(partialFC{i,1}([1:(j-25) (j-23):(j-1) (j+1):48],j)),allALI(i,[1:(j-25) (j-23):(j-1) (j+1):48]))/sum(abs(partialFC{i,1}([1:(j-25) (j-23):(j-1) (j+1):48],j)));
        end
    end
end
...
icc_Pa_r_L=zeros(24,1);
icc_Pa_r_L_p=zeros(24,1);
for i=1:24
    [icc_Pa_r_L(i,1), LB, UB, F, df1, df2, icc_Pa_r_L_p(i,1)] = ICC([ALI_pre_L(:,i),All_ALI_L(:,i)], 'A-1',0.05,0);
end
icc_Pa_r_R=zeros(24,1);
icc_Pa_r_R_p=zeros(24,1);
for i=1:24
