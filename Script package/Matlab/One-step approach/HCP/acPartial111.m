pathD=['H:\HCPdata\LanguageCompute\Txtoutput12\r_value_results2\''partialFC.mat'];
sbj = importdata('H:\HCPdata\LanguageCompute\Txtoutput\sbj.txt'); 
partialFC=cell(100,1);
for t=1:length(sbj)
    data=importdata(['H:\HCPdata\LanguageCompute\Txtoutput12\partialFC\' sbj{t} '.mat']); 
...
for i=1:100
    for j=1:20
        if j==1
            ...
            ALI_pre_L(i,j)=dot(abs(partialFC{i,1}(2:20,j)),All_ALI(i,2:20))/sum(abs(partialFC{i,1}(2:20,j)))  ;
        else
         
            ALI_pre_L(i,j)=dot(abs(partialFC{i,1}([1:j-1,j+1:20],j)),All_ALI(i,[1:j-1,j+1:20]))/sum(abs(partialFC{i,1}([1:j-1,j+1:20],j)))  ;
        end
    end
end
...
for i=1:100
    for j=1:20
        if j==1
            ...
            ALI_pre_L_2(i,j)=dot(abs(partialFC{i,1}(2:20,j+20)),All_ALI(i,2:20))/sum(abs(partialFC{i,1}(2:20,j+20)))  ;
        else
           
            ALI_pre_L_2(i,j)=dot(abs(partialFC{i,1}([1:j-1,j+1:20],j+20)),All_ALI(i,[1:j-1,j+1:20]))/sum(abs(partialFC{i,1}([1:j-1,j+1:20],j+20)));
        end
    end
end
...
for i=1:100
    for j=1:20
        if j==1
            ...
            ALI_pre_R(i,j)=dot(abs(partialFC{i,1}(22:40,j+20)),All_ALI(i,2:20))/sum(abs(partialFC{i,1}(22:40,j+20)))  ;
        else
           
            ALI_pre_R(i,j)=dot(abs(partialFC{i,1}([21:j+19,j+21:40],j+20)),All_ALI(i,[1:j-1,j+1:20]))/sum(abs(partialFC{i,1}([21:j+19,j+21:40],j+20)));
        end
    end
end
...
for i=1:100
    for j=1:20
        if j==1
            ...
            ALI_pre_R_2(i,j)=dot(abs(partialFC{i,1}(22:40,j)),All_ALI(i,2:20))/sum(abs(partialFC{i,1}(22:40,j)))  ;
        else
   
            ALI_pre_R_2(i,j)=dot(abs(partialFC{i,1}([21:j+19,j+21:40],j)),All_ALI(i,[1:j-1,j+1:20]))/sum(abs(partialFC{i,1}([21:j+19,j+21:40],j)));
        end
    end
end
...
for i=1:100
    for j=1:40
        if j == 1
          ...
          ALI_pre(i,j)=dot(abs(partialFC{i,1}([2:20 22:40],1)),allALI(1,[2:20 22:40]))/sum(abs(partialFC{i,1}([2:20 22:40],1))); 
        elseif j<=20
       
           ALI_pre(i,j)=dot(abs(partialFC{i,1}([1:(j-1) (j+1):(j+19) (j+21):40],j)),allALI(i,[1:(j-1) (j+1):(j+19) (j+21):40]))/sum(abs(partialFC{i,1}([1:(j-1) (j+1):(j+19) (j+21):40],j)));
        else 
           ALI_pre(i,j)=dot(abs(partialFC{i,1}([1:(j-21) (j-19):(j-1) (j+1):40],j)),allALI(i,[1:(j-21) (j-19):(j-1) (j+1):40]))/sum(abs(partialFC{i,1}([1:(j-21) (j-19):(j-1) (j+1):40],j)));
        end
    end
end
...
icc_Pa_p=zeros(20,1);
for i=1:20
    [icc_Pa(i,1), LB, UB, F, df1, df2,icc_Pa_p(i,1)] = ICC([AA(:,i),All_ALI_L(:,i)], 'A-1',0.05,0);
end
icc_Pa_N=zeros(5,1);
icc_Pa_N_p=zeros(5,1);
for i=1:5
    [icc_Pa_N(i,1), LB, UB, F, df1, df2, icc_Pa_N_p(i,1)] = ICC([A_pre(:,i),NT(:,i)], 'A-1',0.05,0);
end
icc_Pa_r_L=zeros(20,1);
icc_Pa_r_L_p=zeros(20,1);
for i=1:20
    [icc_Pa_r_L(i,1), LB, UB, F, df1, df2, icc_Pa_r_L_p(i,1)] = ICC([ALI_pre_L(:,i),All_ALI_L(:,i)], 'A-1',0.05,0);
end
icc_Pa_r_R=zeros(20,1);
icc_Pa_r_R_p=zeros(20,1);
for i=1:20
    [icc_Pa_r_R(i,1), LB, UB, F, df1, df2, icc_Pa_r_R_p] = ICC([ALI_pre_R(:,i),All_ALI_L(:,i)], 'A-1',0.05,0);
end
save(pathD,'r_L','r_R','R6','R1','R2','R7','p_L','p_R','P6','P1','P2','P7','Ml','Mr','M','All_ALI_L','ALI_pre_L','ALI_pre_R','AA','NT','N_pre_L','N_pre_R','A_pre','partialFC','icc_Pa','icc_Pa_p','icc_Pa_N','icc_Pa_N_p'); 
Pa_R6=R6;
Pa_R7=R7;