pathD=['H:\HCPdata\LanguageCompute\Txtoutput12\r_value_results2\''CombinedFC.mat'];
sbj = importdata('H:\HCPdata\LanguageCompute\Txtoutput\sbj.txt'); 
CombinedFC=cell(100,1);
for t=1:length(sbj)
    data=importdata(['H:\HCPdata\LanguageCompute\Txtoutput12\CombinedFC\' sbj{t} '.mat']); 
    data1=data([2 4 5 7:11 14 16:26 28 30 31 33:37 40 42:52],[2 4 5 7:11 14 16:26 28 30 31 33:37 40 42:52]);
    CombinedFC{t,1}=data1;
end
...
for i=1:100
    for j=1:20
        if j==1
            ...
            ALI_pre_L(i,j)=dot(abs(CombinedFC{i,1}(2:20,j)),All_ALI(i,2:20))/sum(abs(CombinedFC{i,1}(2:20,j)))  ;
        else
            ALI_pre_L(i,j)=dot(abs(CombinedFC{i,1}([1:j-1,j+1:20],j)),All_ALI(i,[1:j-1,j+1:20]))/sum(abs(CombinedFC{i,1}([1:j-1,j+1:20],j)))  ;
        end
    end
end
...
for i=1:100
    for j=1:20
        if j==1
            ...
            ALI_pre_L_2(i,j)=dot(abs(CombinedFC{i,1}(2:20,j+20)),All_ALI(i,2:20))/sum(abs(CombinedFC{i,1}(2:20,j+20)))  ;
        else
       
            ALI_pre_L_2(i,j)=dot(abs(CombinedFC{i,1}([1:j-1,j+1:20],j+20)),All_ALI(i,[1:j-1,j+1:20]))/sum(abs(CombinedFC{i,1}([1:j-1,j+1:20],j+20)));
        end
    end
end
...
for i=1:100
    for j=1:20
        if j==1
            ...
            ALI_pre_R(i,j)=dot(abs(CombinedFC{i,1}(22:40,j+20)),All_ALI(i,2:20))/sum(abs(CombinedFC{i,1}(22:40,j+20)))  ;
        else
           
            ALI_pre_R(i,j)=dot(abs(CombinedFC{i,1}([21:j+19,j+21:40],j+20)),All_ALI(i,[1:j-1,j+1:20]))/sum(abs(CombinedFC{i,1}([21:j+19,j+21:40],j+20)));
        end
    end
end
...
for i=1:100
    for j=1:20
        if j==1
            ...
            ALI_pre_R_2(i,j)=dot(abs(CombinedFC{i,1}(22:40,j)),All_ALI(i,2:20))/sum(abs(CombinedFC{i,1}(22:40,j)))  ;
        else
   
            ALI_pre_R_2(i,j)=dot(abs(CombinedFC{i,1}([21:j+19,j+21:40],j)),All_ALI(i,[1:j-1,j+1:20]))/sum(abs(CombinedFC{i,1}([21:j+19,j+21:40],j)));
        end
    end
end
...
for i=1:100
    for j=1:40
        if j == 1
          ...
          ALI_pre(i,j)=dot(abs(CombinedFC{i,1}([2:20 22:40],1)),allALI(1,[2:20 22:40]))/sum(abs(CombinedFC{i,1}([2:20 22:40],1))); 
        elseif j<=20
           
           ALI_pre(i,j)=dot(abs(CombinedFC{i,1}([1:(j-1) (j+1):(j+19) (j+21):40],j)),allALI(i,[1:(j-1) (j+1):(j+19) (j+21):40]))/sum(abs(CombinedFC{i,1}([1:(j-1) (j+1):(j+19) (j+21):40],j)));
        else 
           ALI_pre(i,j)=dot(abs(CombinedFC{i,1}([1:(j-21) (j-19):(j-1) (j+1):40],j)),allALI(i,[1:(j-21) (j-19):(j-1) (j+1):40]))/sum(abs(CombinedFC{i,1}([1:(j-21) (j-19):(j-1) (j+1):40],j)));
        end
    end
end
...
icc_Co_r_L=zeros(20,1);
icc_Co_r_L_p=zeros(20,1);
for i=1:20
    [icc_Co_r_L(i,1), LB, UB, F, df1, df2, icc_Co_r_L_p(i,1)] = ICC([ALI_pre_L(:,i),All_ALI_L(:,i)], 'A-1',0.05,0);
end
icc_Co_r_R=zeros(20,1);
icc_Co_r_R_p=zeros(20,1);
for i=1:20
