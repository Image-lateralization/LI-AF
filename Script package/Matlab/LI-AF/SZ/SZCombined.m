pathD=['H:\WM_data\SZ\BOLDextract\Txtoutput5\r_value_results\''CombinedFC.mat'];
sbj = importdata('H:\WM_data\RESTSZ\Txtoutput\RESTSZsbj.txt'); 
CombinedFC=cell(37,1);
for t=1:length(sbj)
    data=importdata(['H:\WM_data\SZ\BOLDextract\Txtoutput4\CombinedFC\' sbj{t} '.mat']);
    CombinedFC{t,1}=data;
end
...
for i=1:37
    for j=1:48
        if j == 1
           ALI_pre(i,j)=dot(abs(CombinedFC{i,1}(2:48,1,1)),allALI(1,2:48))/sum(abs(CombinedFC{i,1}(2:48,1,1)))  ;
        else
           ALI_pre(i,j)=dot(abs(CombinedFC{i,1}([1:(j-1) (j+1):48],j)),allALI(i,[1:(j-1) (j+1):48]))/sum(abs(CombinedFC{i,1}([1:(j-1) (j+1):48],j)))  ;
        end
    end
end
