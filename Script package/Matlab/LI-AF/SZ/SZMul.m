pathD=['H:\WM_data\SZ\BOLDextract\Txtoutput5\r_value_results\''mulregressionFC.mat'];
sbj = importdata('H:\WM_data\RESTSZ\Txtoutput\RESTSZsbj.txt'); 
for t=1:length(sbj)
    
    data=importdata(['F:\WM_data\SZ\BOLDextract\Txtoutput4\mulregressionFC\' sbj{t} '.mat']); 
    mulFC{t,1}=data1;
end
...
for i=1:37
    for j=1:48
        if j == 1
           ALI_pre(i,j)=dot(abs(mulFC{i,1}(2:48,1,1)),allALI(1,2:48))/sum(abs(mulFC{i,1}(2:48,1,1)))  ;
        else
           ALI_pre(i,j)=dot(abs(mulFC{i,1}([1:(j-1) (j+1):48],j)),allALI(i,[1:(j-1) (j+1):48]))/sum(abs(mulFC{i,1}([1:(j-1) (j+1):48],j)));
        end
    end
end
...
mmAA=mean(AA);
mmmALI=mean(All_ALI_L);
[rrr,ppp]=corr(mmAA',mmmALI');
