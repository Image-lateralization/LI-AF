pathD=['H:\HCPdata\LanguageCompute\Txoutput2\r_value_results\''mulregressionFC.mat'];
sbj = importdata('H:\HCPdata\LanguageCompute\Txtoutput\sbj.txt');
mulFC=cell(100,1);
for t=1:length(sbj)
   
    data=importdata(['F:\HCPdata\LanguageCompute\Txtoutput\mulregressionFC\' sbj{t} '.mat']); 
...
for i=1:100
    for j=1:40
        if j == 1
           ...
           ALI_pre(i,j)=dot(abs(mulFC{i,1}(2:40,1,1)),allALI(1,2:40))/sum(abs(mulFC{i,1}(2:40,1,1)))  ;
        else
           ALI_pre(i,j)=dot(abs(mulFC{i,1}([1:(j-1) (j+1):40],j)),allALI(i,[1:(j-1) (j+1):40]))/sum(abs(mulFC{i,1}([1:(j-1) (j+1):40],j)))  ;
        end
    end
end
...
mean(rr)
save(pathD,'r_L','r_R','R6','R1','R2','R7','p_L','p_R','P6','P1','P2','P7','Ml','Mr','M','All_ALI_L','ALI_pre_L','ALI_pre_R','AA','NT','N_pre_L','N_pre_R','A_pre','mulFC'); 