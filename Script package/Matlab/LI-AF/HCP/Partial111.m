pathD=['H:\HCPdata\LanguageCompute\Txoutput2\r_value_results\''partialFC.mat'];
sbj = importdata('H:\HCPdata\LanguageCompute\Txtoutput\sbj.txt'); 
partialFC=cell(100,1);
for t=1:length(sbj)
    data=importdata(['H:\HCPdata\LanguageCompute\Txtoutput\partialFC\' sbj{t} '.mat']); 
...
for i=1:100
    for j=1:40
        if j == 1
           ...
           ALI_pre(i,j)=dot(abs(partialFC{i,1}(2:40,1,1)),allALI(1,2:40))/sum(abs(partialFC{i,1}(2:40,1,1)))  ;
        else
           ALI_pre(i,j)=dot(abs(partialFC{i,1}([1:(j-1) (j+1):40],j)),allALI(i,[1:(j-1) (j+1):40]))/sum(abs(partialFC{i,1}([1:(j-1) (j+1):40],j)))  ;
        end
    end
end
