%pathD=['H:\WM_data\HC\BOLDextract\Txtoutput5\r_value_results\''partialFC.mat'];
sbj = importdata('H:\WM_data\HC\BOLDextract\Txtoutput4\HCsbj.txt'); 
partialFC=cell(88,1);
for t=1:length(sbj)
    data=importdata(['H:\WM_data\HC\BOLDextract\Txtoutput4\partialFC\' sbj{t} '.mat']);
    partialFC{t,1}=data;
end
...
for i=1:88
    for j=1:48
        if j == 1
           ALI_pre(i,j)=dot(abs(partialFC{i,1}(2:48,1,1)),allALI(1,2:48))/sum(abs(partialFC{i,1}(2:48,1,1)));
        else
           ALI_pre(i,j)=dot(abs(partialFC{i,1}([1:(j-1) (j+1):48],j)),allALI(i,[1:(j-1) (j+1):48]))/sum(abs(partialFC{i,1}([1:(j-1) (j+1):48],j)));
        end
    end
end
...
mean(rr)
%save(pathD,'r_L','r_R','R6','R1','R2','R7','p_L','p_R','P6','P1','P2','P7','Ml','Mr','M','All_ALI_L','ALI_pre_L','ALI_pre_R','AA','NT','N_pre_L','N_pre_R','A_pre','partialFC'); 
