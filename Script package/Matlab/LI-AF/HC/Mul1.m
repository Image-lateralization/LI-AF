%pathD=['H:\WM_data\HC\BOLDextract\Txtoutput5\r_value_results\''mulregressionFC.mat'];
sbj = importdata('H:\WM_data\HC\BOLDextract\Txtoutput4\HCsbj.txt'); 
mulFC=cell(88,1);
for t=1:length(sbj)
    data=importdata(['H:\WM_data\HC\BOLDextract\Txtoutput4\mulregressionFC\' sbj{t} '.mat']); 
    mulFC{t,1}=data;
...
for i=1:88
    for j=1:48
        if j == 1
           ALI_pre(i,j)=dot(abs(mulFC{i,1}(2:48,1,1)),All_ALI(1,2:48))/sum(abs(mulFC{i,1}(2:48,1,1))) ;
        else
           ALI_pre(i,j)=dot(abs(mulFC{i,1}([1:(j-1) (j+1):48],j)),All_ALI(i,[1:(j-1) (j+1):48]))/sum(abs(mulFC{i,1}([1:(j-1) (j+1):48],j)));
        end
    end
end
R=zeros(1,48);
