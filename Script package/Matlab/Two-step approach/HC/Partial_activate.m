pathD=['H:\WM_data\HC\BOLDextract\Txtoutput11\r_value_results2\''partialFC.mat'];
...
for i=1:88
    for j=1:48
        if j == 1
           Activate_pre(i,j)=dot(partialFC{i,1}(2:48,1,1),All_activate(1,2:48)) ;
        else
           Activate_pre(i,j)=dot(partialFC{i,1}([1:(j-1) (j+1):48],j),All_activate(i,[1:(j-1) (j+1):48]));
        end
    end
end
